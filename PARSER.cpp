...
int main() {
    ...
    do{
    	char **symboltbl_rowFields = CsvParser_getFields(symboltbl_row);;
    	printf("\n\nSTACK'S TOP: %s", get_top());
    	printf("\nCURRENT TOKEN: %s\nACTION--------", symboltbl_rowFields[1]);
    	
    	
    	if(strcmp(symboltbl_rowFields[1], "COMMENT_OPERATOR") == 0 || strcmp(symboltbl_rowFields[1], "SINGLE_COMMENT") == 0 || strcmp(symboltbl_rowFields[1], "MULTI_COMMENT") == 0){
		...
		else if((terminal_identifier(get_top())==1)){ //top of stack is symbol
